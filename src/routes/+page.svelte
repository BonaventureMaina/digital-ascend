<script lang="ts">
  ...
  async function handleSubmit(event: Event) {
    ...
    try {
      ...
      if (response.ok) { submitted = true; error = ''; }
      else { const data = await response.json(); error = data.error || 'Something went wrong. Please try again.'; }
...
<div class="min-h-[90vh] flex flex-col items-center justify-center bg-gradient-to-b from-gray-900 via-gray-900 to-indigo-950 text-white">
  ...
  <div class="relative z-10 max-w-3xl mx-auto text-center px-4 py-16">
    ...
    <h1 class="text-5xl md:text-6xl lg:text-7xl font-bold mb-6 leading-tight">Digital Ascend<span class="block text-2xl md:text-3xl lg:text-4xl font-light mt-2 text-gray-300">Arts Production</span></h1>
    <p class="text-lg md:text-xl text-gray-400 max-w-2xl mx-auto mb-8">Original one‑act plays for the stage and the screen.<br />Debut production: <em class="text-amber-400 not-italic">The Last Row</em> — coming soon.</p>
    <div class="mb-10">
    ...
    <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-6 max-w-md mx-auto mb-10">
      ...
      <p class="text-sm text-gray-400 mb-4">Be the first to know about new scenes, ticket releases, and streaming announcements.</p>
      {#if submitted}
        <p class="text-green-400 bg-green-400/10 border border-green-400/30 rounded-lg p-3 text-sm">You're on the list! Thank you.</p>
      {:else}
        <form onsubmit={handleSubmit} class="flex flex-col sm:flex-row gap-3">
          <input type="email" name="email" placeholder="your@email.com" required class="flex-1 px-4 py-2.5 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-amber-400/50" />
          <button type="submit" class="bg-amber-500 hover:bg-amber-400 text-gray-900 font-semibold px-6 py-2.5 rounded-lg transition-colors">Notify Me</button>
        </form>
