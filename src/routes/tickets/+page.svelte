  ...
  <div class="max-w-xl mx-auto px-4 py-16">
    ...
    {#if success() && booking()}
      ...
      <div class="bg-green-400/10 border border-green-400/30 rounded-2xl p-6 backdrop-blur-sm">
        ...
        <button
          ...
          class="mt-4 bg-amber-500 hover:bg-amber-400 text-gray-900 font-semibold px-6 py-2.5 rounded-lg transition-colors"
        >
          Print Ticket
        </button>
      </div>
    {:else}
      ...
      <form method="POST" class="space-y-5 bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
        ...
        <button type="submit" class="w-full bg-amber-500 hover:bg-amber-400 text-gray-900 font-semibold px-6 py-3 rounded-lg transition-colors">
          Book Now (Simulated)
        </button>
      </form>
    {/if}
